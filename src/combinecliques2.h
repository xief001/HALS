...
typedef struct clique_node//clique 节点的结构：数据部分存储clique名称，指针部分指向下一个clique
{
	string clique;
	int clique_ID;
...
typedef struct clique_list//clique 表的结构：数据部分存储clique名称，指针部分指向read名称
{
	string clique_name;
	int clique_ID;
	int read_num;
...
typedef struct read_list//read 表的结构：数据部分存储read名称，指针部分指向clique名称
{
	string read_name;
	int read_ID;
	int flag;
